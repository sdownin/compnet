...
# {
  ...
  for (t in 1:npds) {
    for (i in 1:length(firmnamesub)) {
      ...
      xr <- ceiling( log2( 1 + xr ) )
      arrNetR[i,t] <-  ifelse( xr > cut.r, cut.r, xr)
      ## RESTRUCT
      xi <- dfla$rp_net_invariant[idx]
      xi <- ceiling( log2( 1 + xi ) )
      arrNetR[i,t] <-  ifelse( xi > cut.i, cut.i, xi)
      ## Acquisitions
  ...
  ##
  firmsubidx <- which(firmnamesub %in% intersect(nmsale,nmactall)) ## ## FILTER ACQUISITIONS > 0
  ...
  arrAcqSq2 <- arrAcqSq[firmsubidx, yridx ]
  #
  arrAcq2 <- arrAcq[firmsubidx, yridx ]
  arrAcqSq2 <- arrAcqSq[firmsubidx, yridx ]
  ##-----------------------------
  ...
  sysEff <- includeEffects(sysEff, avAlt,
                           name="depNetR", interaction1 = 'depMMC')
  sysEff <- includeInteraction(sysEff, outdeg, avAlt,
  ...
  # 
  summary(sysResAMC0); screenreg(sysResAMC0, digits = 4,single.row = T)
