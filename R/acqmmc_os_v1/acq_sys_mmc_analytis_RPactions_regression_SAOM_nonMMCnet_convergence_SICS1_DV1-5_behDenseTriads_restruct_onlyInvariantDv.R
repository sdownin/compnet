...
# {
  ...
  sysEff <- includeEffects(sysEff, egoX,
                           name="depMMC", interaction1 = 'covSales')
  sysEff <- includeEffects(sysEff, egoX,
                           name="depMMC", interaction1 = 'covAcqExper')
  sysEff <- includeEffects(sysEff, egoX,
  ...
  # sysEff <- includeEffects(sysEff, diffX,
  #                          name="depMMC", interaction1 = 'depNetR')
  ...
  sysEff <- includeEffects(sysEff, totAlt,
                           name="depNetI", interaction1 = 'depMMC')
  #
  sysEff <- includeEffects(sysEff, behDenseTriads,
                           name="depNetI", interaction1 = 'depMMC')
  #
  sysEff <- includeInteraction(sysEff, behDenseTriads, totAlt,
                               name="depNetI", interaction1 = c('depMMC','depMMC'))
  sysEff <- includeInteraction(sysEff, behDenseTriads, totAlt,
                               name="depNetR", interaction1 = c('depMMC','depMMC'))
  #
  sysMod <- sienaAlgorithmCreate(projname='sys-mmc-acq-test-proj-converg-VDcut-behDenseTriad_restruct_1-Inf_H1ab_H2-0_onlyInvariantDV', 
  ...
  .prefix <- 'SAOM_1-Inf_H1ab_H2_onlyInvariantDV_GOF'
  ...
  saveRDS(list(res=sysResAMC0, mod=sysMod, dat=sysDat, eff=sysEff),
          file = 'acq_sys_mmc_sysResAMC0_converg_DVcut_behDenseTriad_restruct_1-Inf_H1ab_H2_onlyInvariantDV.rds')
...
### 
  ...
  mc0 <- c('depNetR ego',
           'depNetR isolate', 'depNetR degree', 'depNetR total alter',
           'depNetI isolate', 'depNetI degree', 'depNetI total alter')
  mc1 <- c('depNetI dense triads','depNetR dense triads')
  ...
  mtf$pvalue <- as.numeric( round(mtf$pvalue, 7) )
  print(mtf)

  mc0 <- c('depNetR ego',
           'depNetR isolate', 'depNetR degree', 'depNetR total alter',
           'depNetI isolate', 'depNetI degree', 'depNetI total alter',
           'depNetR dense triads', 'depNetR dense triads')
  mc1 <- c('depNetI dense triads')
  ...
  ##
  mth <- ldply(list(
    Multipar.RSiena(sysResH1ab, 25),
    # Multipar.RSiena(sysResH2, 11),
    Multipar.RSiena(sysResAMC0, 27)
  ), data.frame)
