...
# {
  ...
  ##----------------------------------------
  # workspace_file <- sprintf(sprintf('sys_mmc_workspace_pre-saom_wStratTact_%s_%s-%s.RData',
  ...
  ######
  par(mfrow=c(1,1))
  behdf <-              within(plyr::count(c(arrStr2[])),{beh<-'Strategic'; MarketGrowth<-F })
  behdf <- rbind(behdf, within(plyr::count(c(arrTac2[])),{beh<-'Tactical'; MarketGrowth<-F }))
  behdf <- rbind(behdf, within(plyr::count(c(arrStrw2[])),{beh<-'Strategic'; MarketGrowth<-T }))
  behdf <- rbind(behdf, within(plyr::count(c(arrTacw2[])),{beh<-'Tactical'; MarketGrowth<-T }))
  ...
  behyrdf <- within(plyr::count(c(arrStrw2[,1])),{beh<-'Strategic'; year<-netwavepds[yridx[1]] })
  for (tt in 2:length(yridx)) 
    behyrdf <- rbind(behyrdf, within(plyr::count(c(arrStrw2[,tt])),{beh<-'Strategic'; year<-netwavepds[yridx[tt]] }) )
  behyrdf <- rbind(behyrdf, within(plyr::count(c(arrTacw2[,1])),{beh<-'Tactical'; year<-netwavepds[yridx[1]] }) )
  for (tt in 2:length(yridx)) 
    behyrdf <- rbind(behyrdf, within(plyr::count(c(arrTacw2[,tt])),{beh<-'Tactical'; year<-netwavepds[yridx[tt]] }) )
  ...
  ##
  ##-----------------------------------------------------------------------
  sysDat <- sienaDataCreate(depMMC, depStr, depTac,  #depNonAcq, depNetR, depNetI
  ...
  sysEff <- includeEffects(sysEff, totDist2, 
                              name="depMMC", type='creation', interaction1 = 'depStr')
  ## CONTROL BEHVAIOR
  sysEff <- includeEffects(sysEff, isolate,
                           name="depTac", interaction1 = 'depMMC')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depTac", interaction1 = 'covEmploy')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depTac", interaction1 = 'covSales')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depTac", interaction1 = 'covSlack')
  #
  sysEff <- includeEffects(sysEff, isolate,
                           name="depStr", interaction1 = 'depMMC')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depStr", interaction1 = 'covAcqExper')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depStr", interaction1 = 'covEmploy')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depStr", interaction1 = 'covSales')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depStr", interaction1 = 'covSlack')
  ## BEHAVIOR
  ...
  sysEff <- includeEffects(sysEff, behDenseTriads,# behDenseTriads,
                           name="depTac", interaction1 = 'depMMC')
  #
  sysMod <- sienaAlgorithmCreate(projname='sys-mmc-acq-test-proj-converg-VDcut-behDenseTriad_stratTact-0', 
                                 firstg = 0.15,  ## default: 0.2
  ...
  saveRDS(list(res=sysResAMC0, mod=sysMod, dat=sysDat, eff=sysEff),
          file = 'acq_sys_mmc_sysResAMC0_H1ab_H2_converg_DVcut_behDenseTriad_stratTact.rds')
