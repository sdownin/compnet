...
# {
  ...
  for (t in 1:npds) {
    ...
    for (i in 1:length(firmnamesub)) {
      ...
      #
      # Scale Actions
      denom <- log10( 1 + xti$sales_na_0_mn )
      denom2 <- log10( 1 + mean(c(xti$sales_na_0_mn,  xtin1$sales_na_0_mn)))
      #
      .tmp <- data.frame(
        ...
        .sl1 <- xtin1$sales_na_0_mn[1],
        .s   <- xti$sales_na_0_mn[1],
        sales_na_0_mn_diff1bin=ifelse(.s > .sl1, 1, ifelse(.s < .sl1, -1, 0)),
        acq_cnt_5= xti$acq_cnt_5[1],
        acq_sum_1= xti$acq_sum_1[1],
        cent_deg_mmc= xti$cent_deg_mmc[1],
        ## DV Acquisitions
        rp_Acquisitions   =xti$rp_Acquisitions,
        ...
        rp_all   =xti$rp_all,
        rp_all_ma=mean(c(xti$rp_all,
                         xtin1$rp_all,
                         xtin2$rp_all), na.rm=T),
        rp_NON_acquisitions   =xti$rp_NON_acquisitions,
        rp_NON_acquisitions_ma=mean(c(xti$rp_NON_acquisitions,
                                      xtin1$rp_NON_acquisitions,
                                      xtin2$rp_NON_acquisitions), na.rm=T),
        ## behavior scaled
        rp_Acquisitions_s=    xti$rp_Acquisitions / denom,   ## Acq
        rp_NON_acquisitions_s=xti$rp_NON_acquisitions / denom ,   ## 
        rp_all_s=             xti$rp_all / denom,   ## 
        ##
        rp_Acquisitions_s_ma2=    mean(c(xti$rp_Acquisitions, xtin1$rp_Acquisitions)) / denom2,   ## Acq
        rp_NON_acquisitions_s_ma2=mean(c(xti$rp_NON_acquisitions, xtin1$rp_NON_acquisitions)) / denom2,   ## 
        rp_all_s_ma2=             mean(c(xti$rp_all, xtin1$rp_all)) / denom2,   ## 
  ...
  # dfla$sales_na_0_mn_ldiff[dfla$sales_na_0_mn_ldiff==Inf] <- max(dfla$sales_na_0_mn_ldiff[which(dfla$sales_na_0_mn_ldiff < Inf)])
  ...
  arrSalesSoc <- array(0, dim=c(nsub, nsub, nwaves)) ## relative social performance
  #
  for (t in 1:nwaves) {
    wave <- netwavepds[t]
    cat(sprintf('wave %s\n',wave))
    ...
    ##---SMMC-CENTRALITY-----------------
    # arrSmmc[,t] <-  igraph::degree(gtsubmmc)
    # arrSmmc[,t] <- log( 1 + igraph::degree(gtsubmmc) )
    arrSmmc[,t] <- log(1 + igraph::authority_score(gtsubmmc.w)$vector * 100)
    # arrSmmc[,t] <- log(1 + igraph::authority_score(gtsubmmc)$vector * 100)
    # arrSmmc[,t] <- log( 1 + igraph::power_centrality(gtsubmmc, exponent = 0)*100 )
    # arrSmmc[,t] <- log(  igraph::subgraph.centrality(gtsubmmc.w, diag = F) )
    # arrSmmc[,t] <- igraph::coreness(gtsubmmc.w)
    # arrSmmc[,t] <- log (1 + 100 / igraph::eccentricity(gtsubmmc.w) )
    # arrSmmc[,t] <- igraph::transitivity(gtsubmmc, type = 'local', isolates='zero') * 100
    ...
    # arrSmmc[,t] <-  log( 1 + igraph::eigen_centrality(gtsubmmc.w)$vector * 100)
    # arrSmmc[,t] <- log(1 + authority_score(gtsubmmc)$vector*100)
    # const <-  constraint(gtsubmmc.w)
    # const[is.nan(const) | is.na(const)] <- 1
    # arrSmmc[,t] <- const
    # const <- constraint(gtsubmmc) * 100
    # const[is.nan(const) | is.na(const)] <- 0
    # arrSmmc[,t] <- const
    # eig <- eigen_centrality(gtsubmmc.w)
    # arrSmmc[,t] <- power_centrality(gtsubmmc.w, exponent = 1/max(eig$value))
  ...
  # dfplot$log_acq <- log2(1+dfla$rp_Acquisitions_s) 
  # dfplot$smmc <-  log(1 + dfla$cent_deg_mmc)
  dfplot$sales_high <- as.factor(dfplot$sales_na_0_mn > quantile(dfplot$sales_na_0_mn, probs = .35, na.rm = T))
  # dfplot$sales_high <- as.factor(dfla$rp_Acquisitions > 0)
  # dfplot <- dfplot[which(dfplot$rp_Acquisitions>0),]
  dfplot <- dfplot[which(dfplot$sales_na_0_mn > 0) ,]
  ggplot(dfplot, aes(x=smmc, y=log(1 + rp_Acquisitions), colour=sales_high)) +
  ...
  arrAllAct <- array(0,dim=c(nsub,npds))
  ...
  for (t in 1:npds) {
    for (i in 1:length(firmnamesub)) {
      ...
      thresh <- 5 ## cap behavoir at threshold 
      offset <- 0 ## shift beahvoir distribution to the right (start at 0)
      # Scale Actions
      denom <- log2( 2 + dfla$sales_na_0_mn[idx])
      #
      ceilscale <- 1 #5
      #
      y <- ceiling( ceilscale * dfla$rp_Acquisitions_s[idx])  ## Acq
      cat(sprintf(' t=%s,i=%s, y=%.3f, ifelse = %.3f\n',t,i,y,ifelse(y > thresh, thresh, y)))
      arrAcq[i,t] <-  ifelse(y > thresh, thresh, y)
      #
      z <- ceiling( ceilscale * dfla$rp_NON_acquisitions_s[idx])
      arrNonAcqAct[i,t] <- ifelse(z > thresh, thresh, z)
      # 
      yz <- ceiling( ceilscale * dfla$rp_all_s[idx])
      arrAllAct[i,t] <- ifelse(yz > thresh, thresh, yz)
      #
      ...
      arrSales[i,t]     <- dfla$sales_na_0_mn_diff1bin[idx]
  ...
  # ## offset to zero
  # arrAcq       <- arrAcq - min(c(arrAcq))
  # arrNonAcqAct <- arrNonAcqAct - min(c(arrNonAcqAct))
  # arrAllAct    <- arrAllAct - min(c(arrAllAct))
  
  ## FILTER YEARS
  yridx <- 1:length(netwavepds) #1:(length(netwavepds)-1) #c(1,3,5,7) # c(1,4,7) 
  ## FILTER FIRMS
  nmsale <- unique(dfla$name[which(dfla$sales_na_0_mn>0)])
  nmacqall <- unique(dfla$name[which(dfla$rp_Acquisitions>0)])
  ##
  firmsubidx <- which(firmnamesub %in% intersect(nmsale,nmacqall)) ## ## FILTER ACQUISITIONS > 0
  ...
  arrAllAct2 <- arrAllAct[firmsubidx, yridx ]
  #
  ##=====================================
  ...
  depNonAcq <- sienaDependent(arrNonAcqAct2, type="behavior", nodeSet=c('FIRMS'), 
                           sparse = FALSE, allowOnly = FALSE)
  depAllAcq <- sienaDependent(arrAllAct2, type="behavior", nodeSet=c('FIRMS'), 
                              sparse = FALSE, allowOnly = FALSE)
  # #PREDICTOR
  ...
  covNonAcq <- varCovar(arrNonAcqAct2, nodeSet="FIRMS")
  ...
  sysDat <- sienaDataCreate(depAcq, depMMC,
                            ...
                            covEmploy, covSales, covAcqExper, covNonAcq,
  ...
  ## NETWORK CONTROL
  sysEff <- includeEffects(sysEff, density, gwesp, #isolateNet,#density #isolateNet
                           name="depMMC")
  ## NETWORK SELECTION
  # sysEff <- includeEffects(sysEff, RateX, #density
  #                          name="depMMC",interaction1 = 'depAcq')
  sysEff <- includeEffects(sysEff, altX, #absDiffX
                           name="depMMC",interaction1 = 'depAcq')
  ##
  # sysEff <- includeEffects(sysEff, altX,
  #                          name="depMMC",interaction1 = 'depNonAcq')
  ##
  # sysEff <- includeEffects(sysEff, effFrom,
  #                          name="depAcq",interaction1 = 'depNonAcq')
  ## CONTROL BEHVAIOR
  sysEff <- includeEffects(sysEff, effFrom,
  ...
  # sysEff <- includeEffects(sysEff, effFrom,
  #                          name="depAcq", interaction1 = 'covNonAcq')
  sysEff <- includeEffects(sysEff, effFrom,
  ...
  #                          name="depAcq", interaction1 = 'covSmmc', interaction2 = 'depMMC')
  # sysEff <- includeEffects(sysEff, avSimEgoX, #degAbsContrX,
  #                          name="depAcq", interaction1 = 'covSmmc', interaction2 = 'depMMC')
  # sysEff <- includeEffects(sysEff, avSimEgoX, #degAbsContrX,
  ...
  # sysEff <- includeEffects(sysEff, avSimEgoX, #degAbsContrX,
  #                          name="depAcq", interaction1 = 'covSmmcSq', interaction2 = 'depMMC')
  # sysEff <- includeTimeDummy(sysEff, effFrom, timeDummy="3",
  #                            name="depAcq", interaction1 = 'covSmmcSq')
  
  # sysEff <- includeTimeDummy(sysEff, avSimEgoX, timeDummy="3",
  #                            name="depAcq", interaction1 = 'covSmmcSq',interaction2 = 'depMMC')
  # 
  ...
  sysResH2c <- siena07(sysMod, data=sysDat, effects=sysEff, 
                       #prevAns = sysResH2c,
  ...
  gf2c.o = RSiena::sienaGOF(sysResH2c, OutdegreeDistribution,
                             varName="depMMC"); plot(gf2c.o)  
  #
  ...
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depAcq", interaction1 = 'covSmmcSq')
  # sysEff <- includeEffects(sysEff, effFrom,
  #                          name="depAcq", interaction1 = 'covWdegAll')
  # sysEff <- includeEffects(sysEff, effFrom,
  #                          name="depAcq", interaction1 = 'covSmmc_covWdegAll')
  # sysEff <- includeEffects(sysEff, effFrom,
  #                          name="depAcq", interaction1 = 'covSmmcSq_covWdegAll')
  # ## BEHAVIOR INFLUENCE
  sysEff <- includeEffects(sysEff, avSimEgoX, #degAbsContrX,
                           name="depAcq", interaction1 = 'covSmmcSq', interaction2 = 'depMMC')
  sysEff <- includeTimeDummy(sysEff, effFrom, timeDummy="3",
