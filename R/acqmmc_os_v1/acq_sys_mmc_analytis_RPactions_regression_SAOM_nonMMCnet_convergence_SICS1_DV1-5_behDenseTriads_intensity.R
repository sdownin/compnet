...
# {
  ...
  arrTot <- array(0,dim=c(nsub,npds))
  arrTotw <- array(0,dim=c(nsub,npds))
  # arrTotCx <- array(0,dim=c(nsub,npds))
  # arrTotCxw <- array(0,dim=c(nsub,npds))
  #
  ...
  for (t in 1:npds) {
    for (i in 1:length(firmnamesub)) {
      ...
      ## 7. TOTAL INTENSITY ******************************
      x7 <- dfla$rp_Acquisitions[idx] + dfla$rp_NON_acquisitions[idx] 
      x7 <- ceiling( logXf( 1 + x7 ) )
      arrTot[i,t] <-  ifelse( x7 > cut0, cut0, ifelse(x7 < min0, min0, x7))
      #
      x7w <- (dfla$rp_Acquisitions[idx] + dfla$rp_NON_acquisitions[idx]) / dfla$cs_mkgrowth[idx]
      x7w <- ceiling( logXf( 1 + x7w ) )
      arrTotw[i,t] <-  ifelse( x7w > cut0, cut0, ifelse(x7w < min0, min0, x7w))
      # ## 8. COMPLEXITY (of total)-------
      # xt <- dfla$rp_tacti[idx] 
      # xt <- ceiling( logXf( 1 + xt ) )
      # arrTotCx[i,t] <-  ifelse( xt > cut0, cut0, ifelse(xt < min0, min0, xt))
      # #
      # xtw <- dfla$rp_tacti[idx] / dfla$cs_mkgrowth[idx]
      # xtw <- ceiling( logXf( 1 + xtw ) )
      # arrTotCxw[i,t] <-  ifelse( xtw > cut0, cut0, ifelse(xtw < min0, min0, xtw))
      ##-----------------------------------
      ##-----------------------------------
      ## market growth
  ...
  arrTot2 <- arrTot[firmsubidx, yridx ]
  arrTotw2 <- arrTotw[firmsubidx, yridx ]
  #
  ...
  ######
  par(mfrow=c(1,1))
  behdf <-              within(plyr::count(c(arrTot2[])),{beh<-'Intensity'; MarketGrowth<-F })
  behdf <- rbind(behdf, within(plyr::count(c(arrTotw2[])),{beh<-'Intensity'; MarketGrowth<-T }))
  ...
  behyrdf <- within(plyr::count(c(arrTotw2[,1])),{beh<-'Intensity'; year<-netwavepds[yridx[1]] })
  for (tt in 2:length(yridx)) 
    behyrdf <- rbind(behyrdf, within(plyr::count(c(arrTotw2[,tt])),{beh<-'Intensity'; year<-netwavepds[yridx[tt]] }) )
  ...
  depTac <- sienaDependent(arrTacw2, type="behavior", nodeSet=c('FIRMS'), 
                           sparse = FALSE, allowOnly = FALSE)  
  depTot <- sienaDependent(arrTotw2, type="behavior", nodeSet=c('FIRMS'), 
                           sparse = FALSE, allowOnly = FALSE)
  ...
  ##
  ##-----------------------------------------------------------------------
  sysDat <- sienaDataCreate(depMMC, depTot,  #depNonAcq, depNetR, depNetI
  ...
  sysEff <- includeEffects(sysEff, outInAss, nbrDist2, transTies, #inPopSqrt, 
                           name="depMMC")
  ...
  sysEff <- includeEffects(sysEff, transTriads,
                           name="depMMC", type='eval')
  sysEff <- includeEffects(sysEff, transTriads,
                           name="depMMC", type='creation')
  ## CONTROL BEHVAIOR -----------------------------------------
  sysEff <- includeEffects(sysEff, isolate,
                           name="depTot", interaction1 = 'depMMC')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depTot", interaction1 = 'covAcqExper')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depTot", interaction1 = 'covEmploy')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depTot", interaction1 = 'covSales')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depTot", interaction1 = 'covSlack')
  ## BEHAVIOR
  ...
  sysEff <- includeEffects(sysEff, behDenseTriads,# behDenseTriads,
                           name="depTot", interaction1 = 'depMMC')
  #
  sysMod <- sienaAlgorithmCreate(projname='sys-mmc-acq-test-proj-converg-VDcut-behDenseTriad_intensity-0', 
                                 firstg = 0.07,  ## default: 0.2
                                 n2start=160,    ## default: 2.52*(p+7)
                                 nsub = 4,       ## default: 4
                                 seed=135, maxlike=F)
  ...
  saveRDS(list(res=sysResAMC0, mod=sysMod, dat=sysDat, eff=sysEff),
          file = 'acq_sys_mmc_sysResAMC0_H1ab_H2_converg_DVcut_behDenseTriad_intensity.rds')
...
### 
  ...
  ##---------------------------------------------------------------- 
  sysNameMap <- list(
    `rate depNetR (period 1)`='Rate 2010-2011',
    `rate depNetR (period 2)`='Rate 2011-2012',
    `rate depNetR (period 3)`='Rate 2012-2013',
    `rate depNetR (period 4)`='Rate 2013-2014',
    `rate depNetR (period 5)`='Rate 2014-2015',
    `rate depNetR (period 6)`='Rate 2015-2016',
    ...
    #
    `rate depNetI (period 1)`='Rate 2010-2011',
    `rate depNetI (period 2)`='Rate 2011-2012',
    `rate depNetI (period 3)`='Rate 2012-2013',
    `rate depNetI (period 4)`='Rate 2013-2014',
    `rate depNetI (period 5)`='Rate 2014-2015',
    `rate depNetI (period 6)`='Rate 2015-2016',
    ...
    #
    `constant depMMC rate (period 1)`='Rate 2010-2011',
    `constant depMMC rate (period 2)`='Rate 2011-2012',
    `constant depMMC rate (period 3)`='Rate 2012-2013',
    `constant depMMC rate (period 4)`='Rate 2013-2014',
    `constant depMMC rate (period 5)`='Rate 2014-2015',
    `constant depMMC rate (period 6)`='Rate 2015-2016',
    `degree (density)`='Network Density',
    `transitive triads`='Transitive Triads',
    `creation: transitive triads`='Creation of Transitive Triads',
    `endow: transitive triads`='Maintenance of Transitive Triads',
    `GWESP (69)`='GWESC',
