...
## Load RavenPack
g0 <- asIgraph(nets[[1]])
firms.all <- as.character(V(g0)$vertex.names)
...
for (i in 1:length(firms)) {
  ...
  grepidx <- grep(firm_i,rpn$entity_name,ignore.case=T,perl=T)
  if (length(grepidx)==1) {
    ...
    if (all(idxcnt$freq == 1)) {
      ...
      ## else order by descending metric frequency
...
for (i in 1:length(firms.todo)) {
  ...
  ##--------------------------------------------------------
  ##
  ## Create CB-to-RavenPack Name Mapping for Netowork Firms 
  ##
  ##--------------------------------------------------------
  firms <- as.character(sort(unique(V(g.d.sub)$company_name)))
  # RAVENPACK NAMES
  rpn <- unique(rvn[,c('rp_entity_id','entity_name')])
  rpn$name.l <- str_to_lower(rpn$entity_name)
  ## name map filename
  cbrpmapfile <- sprintf('crunchbase_ravenpack_name_mapping_%s_n%s.csv', name_i, length(firms))
  ## load if exists else create and save name map for focal firm comp net
  if (file.exists(file.path(result_dir,cbrpmapfile))) {
    cbrpmap <- read.csv(file.path(result_dir,cbrpmapfile))
  } else {
    # COHORT OF FIRMS
    cbrpmap <- data.frame(
      name = firms,
