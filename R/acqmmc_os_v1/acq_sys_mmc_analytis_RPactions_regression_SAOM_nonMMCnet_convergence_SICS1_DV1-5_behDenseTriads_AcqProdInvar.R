...
# {
  ...
  ######
  par(mfrow=c(1,1))
  behdf <-              within(plyr::count(c(arrAcq2[])),{beh<-'Acquisitions'; MarketGrowth<-F })
  behdf <- rbind(behdf, within(plyr::count(c(arrProd2[])),{beh<-'Products'; MarketGrowth<-F }))
  behdf <- rbind(behdf, within(plyr::count(c(arrNetI2[])),{beh<-'Invariant'; MarketGrowth<-F }))
  behdf <- rbind(behdf, within(plyr::count(c(arrAcqw2[])),{beh<-'Acquisitions'; MarketGrowth<-T }))
  behdf <- rbind(behdf, within(plyr::count(c(arrProdw2[])),{beh<-'Products'; MarketGrowth<-T }))
  ...
  behyrdf <- within(plyr::count(c(arrAcqw2[,1])),{beh<-'Acquisitions'; year<-netwavepds[yridx[1]] })
  for (tt in 2:length(yridx)) 
    behyrdf <- rbind(behyrdf, within(plyr::count(c(arrAcqw2[,tt])),{beh<-'Acquisitions'; year<-netwavepds[yridx[tt]] }) )
  behyrdf <- rbind(behyrdf, within(plyr::count(c(arrProdw2[,1])),{beh<-'Products'; year<-netwavepds[yridx[1]] }) )
  for (tt in 2:length(yridx)) 
    behyrdf <- rbind(behyrdf, within(plyr::count(c(arrProdw2[,tt])),{beh<-'Products'; year<-netwavepds[yridx[tt]] }) )
  ...
  ##
  ##-----------------------------------------------------------------------
  sysDat <- sienaDataCreate(depMMC, depAcq, depProd, #depNetI,  #depNonAcq, depNetR, depNetI
  ...
  sysEff <- includeEffects(sysEff, totDist2, 
                              name="depMMC", type='creation', interaction1 = 'depAcq')
  ## CONTROL BEHVAIOR
  sysEff <- includeEffects(sysEff, isolate,
                           name="depProd", interaction1 = 'depMMC')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depProd", interaction1 = 'covEmploy')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depProd", interaction1 = 'covSales')
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depProd", interaction1 = 'covSlack')
  #
  sysEff <- includeEffects(sysEff, isolate,
  ...
  sysEff <- includeEffects(sysEff, effFrom,
                           name="depAcq", interaction1 = 'covSlack')
  ## BEHAVIOR
  ...
  sysEff <- includeEffects(sysEff, behDenseTriads,# behDenseTriads,
                           name="depProd", interaction1 = 'depMMC')
  #
  # sysEff <- includeEffects(sysEff, isolate, name="depNetI", interaction1 = 'depMMC')
  # sysEff <- includeEffects(sysEff, effFrom, name="depNetI", interaction1 = 'covEmploy')
  # sysEff <- includeEffects(sysEff, effFrom, name="depNetI", interaction1 = 'covSales')
  # sysEff <- includeEffects(sysEff, effFrom, name="depNetI", interaction1 = 'covSlack')
  # sysEff <- includeEffects(sysEff, outdeg, name="depNetI", interaction1 = 'depMMC')
  # sysEff <- includeEffects(sysEff, totAlt, name="depNetI", interaction1 = 'depMMC')
  #
  sysMod <- sienaAlgorithmCreate(projname='sys-mmc-acq-test-proj-converg-VDcut-behDenseTriad_AcqProdInvar-0', 
  ...
  gfAMC0.br = RSiena::sienaGOF(sysResAMC0, BehaviorDistribution,
                              varName="depAcq"); plot(gfAMC0.br)
  gfAMC0.bi = RSiena::sienaGOF(sysResAMC0, BehaviorDistribution,
                               varName="depProd"); plot(gfAMC0.bi)
  ...
  saveRDS(list(res=sysResAMC0, mod=sysMod, dat=sysDat, eff=sysEff),
          file = 'acq_sys_mmc_sysResAMC0_H1ab_H2_converg_DVcut_behDenseTriad_AcqProdInvar.rds')
