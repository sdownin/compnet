...
# {
  ...
  sysEff <- includeEffects(sysEff, homXTransTrip,
                           name="depMMC", type='creation',  interaction1 = 'depNetR')
  ## CONTROL BEHVAIOR
  ...
  sysEff <- includeEffects(sysEff, behDenseTriads,# behDenseTriads,
                           name="depNetI", interaction1 = 'depMMC')
  #
  sysMod <- sienaAlgorithmCreate(projname='sys-mmc-acq-test-proj-converg-VDcut-behDenseTriad_restruct_1-6_H1ab_H2ab_H3ab-0', 
  ...
  saveRDS(list(res=sysResAMC0, mod=sysMod, dat=sysDat, eff=sysEff),
          file = 'acq_sys_mmc_sysResAMC0_H1ab_H2_converg_DVcut_behDenseTriad_restruct_1-6_H1_H2_H3.rds')
...
### 
                                  ...
                                  coord=NA, seed=111,  ...) 
  {
    ...
    idx <- which(igraph::degree(gs)>=min.degree)
    gs <- igraph::induced.subgraph(gs, vids = idx)
    mat <- mat[idx, idx]
    beh <- beh[idx]
    # ## color ------------------------------
    # V(gs)$color <- rgb(.1,.1,.9,.05)  ## light translucent blue
  ...
  plotMmcNetColorBeh(mat1, beh1, netwavepds[yridx.1], legend.title = 'Restruct.', seed=2921)
  plotMmcNetColorBeh(mat2, beh2, netwavepds[yridx.2], legend.title = 'Restruct.', seed=2921)
