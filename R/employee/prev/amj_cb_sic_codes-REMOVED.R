...
if(!('cb' %in% ls())) 
  source(file.path(getwd(),'R','employee','employee_cb_data_prep.R'))
...
### merge stock symbol into crunchbase company datafrmae
...
if(!('namemap' %in% names(cb)))  {
  if (namemap.filename %in% dir()) {
    ...
    source(file.path(getwd(),'R','employee_cb_cs_name_map.R'))
  }
}
