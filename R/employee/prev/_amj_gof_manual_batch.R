...
results_dir <- file.path(work_dir,'employee_rnr2_results\\firmctrl')
...
## @paramst [character] name of function
## @paramna.rm [bool]  Flag to return GOF DF without statistic values with NaN|NA pvals
## @paramplot [bool]  Flag to create plot
## @paramplotname [character] The plot file name (or full path)
##
## @return df [dataframe]
###
gof.batch <- function(l, st,  na.rm=TRUE,
...
## reduce each simulation:  sum of periods 
lsim <- lapply(l[[st]], function(sim.pds) Reduce('+',sim.pds[pdsub]))
...
print(sprintf('%s: obs MMD = %0.1f, sim MMD avg %.1f (%.1f,%.1f), p = %0.3f', 
              st, d.obs, mean(test$d.sim), min(test$d.sim), max(test$d.sim), 
              test$p[length(test$p)]))
