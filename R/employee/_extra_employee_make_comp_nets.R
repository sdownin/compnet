...
net_dir <- file.path(work_dir,'firm_nets_rnr2_repl')  ##  ***REPLICATION dir***
...
aaf    <- source(file.path(version_dir,'employee_awareness_functions.R'))$value    ## aaf: awareness functions
cb     <- source(file.path(version_dir,'employee_cb_data_prep.R'))$value           ## cb : CrunchBase
sdc    <- source(file.path(version_dir,'employee_sdc_coop.R'))$value               ## sdc: Thompson SDC
si     <- source(file.path(version_dir,'employee_firm_size_controls.R'))$value     ## si : size controls from mergent intellect
ih     <- source(file.path(version_dir,'employee_institutional_holdings.R'))$value ## ih : institutional holdings
g.full <- source(file.path(version_dir,'employee_make_full_graph.R'))$value        ## g.full : full competition graph
