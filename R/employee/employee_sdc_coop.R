...
.main.sdc <- function(x=NA)
{
  ...
  # if(!('cb' %in% ls())) 
  #   source(file.path(version_dir,'employee_cb_data_prep.R'))
  ...
  # namemap.filename <- 'employee_cb_cs_name_map.csv'
  # cs.filename <- 'employee_cs_names.csv'
  ...
  #     .ls <- c(.ls,'cs')
  #   } else {
  #     source(file.path(getwd(),'R','employee_cb_cs_name_map.R')) ## which loads employee_cb_cs_name_map.csv if exists, else creates it in employee_cb_cs_name_map.R
  #   }
  #   source(file.path(getwd(),'R','employee_cb_sic_codes.R'))
  # }
  ...
  cb$co$cusip_6 <- substr(cb$co$cusip, 1, 6)
  
  ## use only the crunchbase companies that have a matched cusip
  tmp.co <- cb$co[!is.na(cb$co$cusip_6),c('company_uuid','cusip_6')]
  ...
  for (i in 1:length(req.cols)) {
    ...
    if (req.col %in% names(sdc)) {
      ...
      sdc <- merge(x=sdc, y=tmp.co, by.x=req.col, by.y='cusip_6', all.x=T, all.y=F)
    } else {
      # cat('skipping ')
    }
  }
