...
# employee_awareness_functions.R
#
# Notes:
#  @export list `eaf`
##
...
.main.eaf <- function()
{
  ...
  ##
  # employee_awareness_functions list object
  ##
  eaf <- list()
  ...
  # @return [igraph]
  ##
  eaf$makeGraph <- function(comp,vertdf,name='company_name_unique', 
  ...
  ##
  eaf$generalistIndex <- function(g, memberships)
  {
  ...
  # @return [float[]] The generalist index vector [float[]]
  ##
  eaf$jobsToBeDone <- function(g, memberships)
  {
    if (class(g) != 'igraph') stop("g must be an igraph object")
  ...
  # 
  ##
  eaf$mmcMarketsDf <- function(df, drop=FALSE, ...)
  {
  ...
  # @return [float]
  ##
  eaf$mmcfromMarketConcat <- function(x,y)
  {
  ...
  # 
  ##
  eaf$coopConcatDf <- function(df, drop=FALSE, ...)
  {
  ...
  # @return [float]
  ##
  eaf$coopFromConcat <- function(x,y)
  {
    if (is.na(x) | is.na(y))
      return(0)
    mx <- c(stringr::str_split(x, '[|]', simplify = T))
  ...
  # @return [matrix]
  ##
  eaf$.cov.coop <- function(net, coop, company_uuids, end, ...)
  {
    ...
    coop.outer <- outer(df.m$concat, df.m$concat, Vectorize(eaf$coopFromConcat))
  ...
  # @return [matrix]
  ##
  eaf$.cov.coopPast <- function(net, coop, company_uuids, start, ...)
  {
    cols <- c('company_uuid','date_alliance_terminated','date_effective','date_expired')
    for (col in cols){
      if (!(col %in% names(coop))) stop(sprintf('coop dataframe missing attribute `%s`', col))
  ...
  # @return [matrix]
  ##
  eaf$.cov.mmc <- function(br, firms, end, ...)
  {
    ...
    if (nrow(brsub)==0) {
      ...
      mmc <- as.matrix(outer(df.m$concat, df.m$concat, Vectorize(eaf$mmcfromMarketConcat)))
    }
    cat('done.\n')
    return(mmc)
  }
  ...
  # @return [matrix]
  ##
  eaf$.cov.invdist <- function(g.net)
  {
    D <- as.matrix(igraph::distances(g.net))
    rownames(D) <- NULL
    colnames(D) <- NULL
  ...
  # @return [matrix]
  ##
  eaf$.cov.ipo <- function(net, ipo, end, nameAttr='vertex.names')
  {
  ...
  # @return [float[]]
  ##
  eaf$.cov.similarity <- function(g, method='invlogweighted')
  {
  ...
  # @return [network] 
  ##
  eaf$.cov.centrality <- function(net)
  {
    g <- asIgraph(net)
  ...
  # @return [network] 
  ## 
  eaf$.cov.generalistIndex <- function(net)
  {
    ...
    ## Generalist Index
    net %v% 'genidx_multilevel'  <- eaf$generalistIndex(g, net %v% 'com_multilevel' )
    net %v% 'genidx_infomap'     <- eaf$generalistIndex(g, net %v% 'com_infomap' )
    net %v% 'genidx_walktrap'    <- eaf$generalistIndex(g, net %v% 'com_walktrap' )
    net %v% 'genidx_fastgreedy'  <- eaf$generalistIndex(g, net %v% 'com_fastgreedy' )
    net %v% 'genidx_edgebetween' <- eaf$generalistIndex(g, net %v% 'com_edgebetween' )
    net %v% 'genidx_labelprop'   <- eaf$generalistIndex(g, net %v% 'com_labelprop' )
    ## Count of competitor's niche clusters == 'Jobs to be done' proxy
    net %v% 'njobs_multilevel'  <- eaf$jobsToBeDone(g, net %v% 'com_multilevel' )
    net %v% 'njobs_infomap'     <- eaf$jobsToBeDone(g, net %v% 'com_infomap' )
    net %v% 'njobs_walktrap'    <- eaf$jobsToBeDone(g, net %v% 'com_walktrap' )
    net %v% 'njobs_fastgreedy'  <- eaf$jobsToBeDone(g, net %v% 'com_fastgreedy' )
    net %v% 'njobs_edgebetween' <- eaf$jobsToBeDone(g, net %v% 'com_edgebetween' )
    net %v% 'njobs_labelprop'   <- eaf$jobsToBeDone(g, net %v% 'com_labelprop' )
    return(net)
  }
  ...
  # @return [numeric] 
  ##
  eaf$.cov.firmSize <- function(net, size, size.var, year,
  ...
  #
  ##
  eaf$.cov.categoryCosineSimilarity <- function(net)
  {
  ...
  # #
  # ##
  # eaf$.cov.subcategoryCosineSimilarity <- function(net)
  # {
  #   all_cat_vec <- sort(unique(unlist(strsplit(net %v% 'category_list', '[|]'))))
  #   # cat_g <- sort(unique(unlist(strsplit(net %v% 'category_group_list', '[|]'))))
  #   # cat <- sort(unique(c(cat_s, cat_g)))
  ...
  #
  ##
  eaf$.cov.sharedCompetitor <- function(net)
  {
    adj <- net[,]
    shared <- adj %*% t(adj)
    diag(shared) <- 0
  ...
  # @return [network] net
  ##
  eaf$.cov.sharedInvestor <- function(net, ih, rou, inv_rou, inv, year, off.diagonal.blocks=F)
  {
    attrs <- network::list.vertex.attributes(net)
    if (!'age' %in% attrs | !'ipo_status' %in% attrs) {
      cat(' \n***First set network age and ipo_status status for conditional means.\n\n')
  ...
  # @return [network]
  ##
  eaf$setCovariates <- function(net, start, end,
                                covlist=c('age','mmc','dist','invdist','ipo_status',
  ...
  { 
    if( network::network.edgecount(net) > 0 ) {
      ...
      if ('coop' %in% covlist) 
      {
        ...
        #
        mat.coop <- eaf$.cov.coop(net, coop, V(g)$company_uuid, t2)  ## returns the updated network
        mat.coop.bin <- mat.coop
        mat.coop.bin[mat.coop.bin >= 1] <- 1
        net %n% 'coop' <- mat.coop
        net %n% 'coop_bin' <- mat.coop.bin
        #
        mat.coop.past <- eaf$.cov.coopPast(net, coop, V(g)$company_uuid, t1)  ## returns the updated network
      ...
      if ('employee' %in% covlist) 
      {
        ...
        net %v% 'employee_na_0' <- eaf$.cov.firmSize(net, size, 'employee_all', start, category.group.fill=F, age.group.fill=F)
        net %v% 'employee_na_cat' <- eaf$.cov.firmSize(net, size, 'employee_all', start, category.group.fill=T, age.group.fill=F)
        net %v% 'employee_na_age' <- eaf$.cov.firmSize(net, size, 'employee_all', start, category.group.fill=F, age.group.fill=T)
        net %v% 'employee_na_catage' <- eaf$.cov.firmSize(net, size, 'employee_all', start, category.group.fill=T, age.group.fill=T)
      ...
      if ('sales' %in% covlist)
      {
        ...
        net %v% 'sales_na_0' <- eaf$.cov.firmSize(net, size, 'sales', start, category.group.fill=F, age.group.fill=F)
        net %v% 'sales_na_cat' <- eaf$.cov.firmSize(net, size, 'sales', start, category.group.fill=T, age.group.fill=F)
        net %v% 'sales_na_age' <- eaf$.cov.firmSize(net, size, 'sales', start, category.group.fill=F, age.group.fill=T)
        net %v% 'sales_na_catage' <- eaf$.cov.firmSize(net, size, 'sales', start, category.group.fill=T, age.group.fill=T)
      ...
      if ('cat_cos_sim' %in% covlist)  ## Category Cosine Similarity
      {
        ...
        net %n% 'cat_cos_sim' <- eaf$.cov.categoryCosineSimilarity(net)
        if (verbose) cat('done\n')
      }
      if ('shared_competitor' %in% covlist)  ## CrunchBase Category Cosine Similarity
      {
        if (verbose) cat('computing Shared Competitor  ...')
        net %n% 'shared_competitor' <- eaf$.cov.sharedCompetitor(net)
        if (verbose) cat('done\n')
      }
      if ('shared_investor' %in% covlist)
      {
        if (verbose) cat('computing Shared Investors...')
        net %n% 'shared_investor_nd' <- eaf$.cov.sharedInvestor(net, ih, rou, inv_rou, inv, start, off.diagonal.blocks=F)
        # net %n% 'shared_investor' <- eaf$.cov.sharedInvestor(net, ih, rou, inv_rou, inv, start, off.diagonal.blocks=TRUE)
        if (verbose) cat('done\n')
      }
  ...
  # @return [network]
  ##
  eaf$setEmployeeCovariates <- function(net, start, end,
  ...
  # @return [igraph] 
  ##
  eaf$nodeCollapseGraph <- function(g, acquisitions, remove.isolates=FALSE, verbose=FALSE)
  {
    if (class(g) != 'igraph') stop("g must be an igraph object")
  ...
  # @return [igraph] 
  ##
  eaf$makePdNetwork <- function(net, start, end, 
  ...
  # @return [igraph] 
  ##
  eaf$makePdGraph <- function(g, start, end,
                              isolates.remove = FALSE,
                              edgeCreatedAttr='relation_began_on',
                              edgeDeletedAttr='relation_ended_on',
  ...
  #
  ##
  eaf$getNetEcount <- function(net, symmetric=TRUE, upper.tri.diag=FALSE)
  {
    if(symmetric)  {
  ...
  ##
  eaf$plotCompNetColPredict <- function(gs, focal.firm=NA, cutoff=.9, 
                                        probAttrName='prob',
                                        competitors=NA,
  ...
  ##
  eaf$covSummaryPlot <- function(nets, name_i, net_dir)
  {
...
# # @return [igraph] 
# ##
# eaf$nodeCollapseContractGraph <- function(g, acquisitions, remove.isolates=FALSE, verbose=FALSE)
# {
#   if (class(g) != 'igraph') stop("g must be an igraph object")
#   if (class(acquisitions) != 'data.frame') stop("acquisitions must be a data frame")
